...
| **State Tree** | 状态机 + 行为树 | UE5 内置，关卡流程、交互物体、**AI 行为** |
...
|:-----|:-----|:-----|:---------|:-----|:---------|
| **GI** | `GI_FiveBox` | `AdvancedFriendsGameInstance` | 全局永久 | Session 管理 | [GI_FiveBox](核心类/GI_FiveBox.md) |
| **GM** | `GM_Core` | `GameModeBase` | 关卡级(仅服务器) | 关卡规则 | [GM_Core](核心类/GM_Core.md) |
| **GS** | `GS_Core` | `GameStateBase` | 关卡级 | 关卡状态同步 | [GS_Core](核心类/GS_Core.md) |
| **PS** | `PS_FiveBox` | `GSCModularPlayerState` | 跨关卡保留 | GAS 宿主 | [PS_FiveBox](核心类/PS_FiveBox.md) |
| **PC** | `PC_Core` | `GSCModularPlayerController` | 跨关卡保留 | 输入/UI | [PC_Core](核心类/PC_Core.md) |
| **Character** | `BP_Character_Game` | `GSCModularPlayerStateCharacter` | 关卡级 | 玩家/AI 角色 | [BP_Character_Game](核心类/BP_Character_Game.md) |
| **AIC** | `AIC_Game` | `AAIController` | 关卡级 | AI 控制 | [AIC_Game](核心类/AIC_Game.md) |
...
| `Server_SetPhase(NewPhase)` | 服务器设置阶段，触发 RepNotify + Handle |
| `HandlePhaseChange(NewPhase)` | 广播 OnPhaseChanged + SendStateTreeEvent |
...
```
BAS_Core (蓝图 AttributeSet, 通过 Blueprint Attributes 插件创建)
├── Health (0~100, Clamping 通过 DataTable 实现)
└── Coins (0~∞)
...
> State Tree 是 UE5 内置的分层状态机系统，需启用 `State Tree` + `Gameplay State Tree` 插件。
