...
- [x] 📝 文档审计：属性系统描述统一化
  - [x] AS_Core → BAS_Core（Blueprint Attributes 插件）
  - [x] 添加项目架构说明（C++ 与蓝图混合，原 AS_Core 已删除）
- [x] 🐛 修复蹲下时 Mesh 位置错误问题
  - [x] 排查根本原因：CMC 网络平滑模式在 Simulated Proxy 上覆盖 Mesh 位置
  - [x] 解决方案：禁用 CharacterMovementComponent 的网络平滑模式
- [x] ✨ 蹲下/起身过程添加危险标签
  - [x] Comp_Endurance 监听 Player.Action.Crouching 标签变化
  - [x] HandleCrouching：Apply GE_Moving → Delay 0.2s → Remove GE_Moving
