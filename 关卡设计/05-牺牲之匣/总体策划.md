...
- 参照关卡1的推搡系统（失衡 → QTE）
- 复用 GA_Push、GA_Stagger、GA_Dodge 等能力框架
...
| Phase 1 | 核心类架构 | Comp_Character_Sacrifice, Comp_PC_Sacrifice, AbilitySet_Sacrifice, IMC_Sacrifice, SM_Sacrifice | ❌ |
...
- 基础出生点（BP_BornVol）和终点（BP_FinishLine）
...
- 创建 Comp_Character_Sacrifice（继承 ActorComponent）
  - 实现 HandlePlayerStart（切换碰撞、应用GE_Started、赋予AbilitySet）
  - 实现 HandlePlayerFinish（应用GE_Finish、切换碰撞、清理AbilitySet）
- 创建 Comp_PC_Sacrifice（继承 ActorComponent）
- 创建 AbilitySet_Sacrifice（复用关卡1的 GA_Push、GA_Stagger、GA_Dodge、GA_Jump）
...
- 后半段视觉差异消失
  - 灯光变暗或材质统一
- **分段切换机制**：
  - 居中玻璃对触发事件：`Gameplay.Event.Level.PhaseChange`
  - SM_Sacrifice 监听事件，从 FirstHalf 切换到 SecondHalf
  - GS_Sacrifice.IsObservablePhase 变量控制视觉差异显示
...
- 复用关卡1的 GA_Push、GA_Stagger、GA_Dodge
