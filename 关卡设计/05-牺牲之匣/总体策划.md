...
- 后半段视觉差异消失
  - 灯光变暗或材质统一
- **分段切换机制**：
  - 玩家踩到中点玻璃时，BP_GlassPanel 通过 IsMid 判断并调用 GS_Sacrifice.SwitchToSecondHalf()
...
- **创建关卡5专属的 GAS 能力**（参考关卡1，但不直接复用）：
  ...
  - GA_Sacrifice_Jump（参考 GA_Jump 的实现方式，但设计完全不同——技巧性跳跃，详见 [跳跃系统.md](GAS/跳跃系统.md)）
