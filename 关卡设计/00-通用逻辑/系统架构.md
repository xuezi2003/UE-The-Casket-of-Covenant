...
### 玩家登录流程
```
玩家连接 → OnPostLogin → GetUniquePlayerNum → RandomAvatar → PS 设置 → GI_FiveBox.AddPlayerRecord → HandlePlayerLogin（绑定事件监听）
```
...
玩家到达终点 → BP_FinishLine.OnComponentEndOverlap
    └─ 发送 Gameplay.Event.Player.Finished
        ↓
        ├─ GM_Core.HandlePlayerFinish（监听事件）
        │   ├─ GI_FiveBox.SetPlayerFinished(PlayerNum)
        │   └─ GS_Core.CheckLevelShouldEnd()
        │
        └─ Comp_Character_Endurance.HandlePlayerFinish（监听事件）
...
| **GM_Core** | 玩家登录、出生、AI 管理、**淘汰/完成统一管理** | OnPostLogin, HandlePlayerLogin, HandlePlayerEliminate, HandlePlayerFinish |
...
| **BP_Character_Game** | 角色移动、外观表现 | InitPlayer, UpdateAvatar |
...
- GM 管理档案（SetPlayerEliminated/SetPlayerFinished）
