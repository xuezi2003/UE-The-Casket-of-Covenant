...
- 每个关卡经历三个阶段：准备 → 进行 → 结算，由 [ST_LevelFlow_Main](核心类/ST_LevelFlow_Main.md) 管理
...
- 不设计跨关卡道具，简化系统复杂度
- 详细属性配置见 [输入与属性.md](输入与属性.md)

## 核心架构

> 详细说明见 [系统架构.md](系统架构.md)

- **档案驱动实体**：[GI_FiveBox](核心类/GI_FiveBox.md) 存储数据，PlayerState/Character 表现状态
- **GM 配置驱动**：各关卡通过 GM 子类配置组件、行为树、技能集，详见 [GM_Core.md](核心类/GM_Core.md)
- **状态树流程**：Preparing（WaitingStart → SetLock → FillAI）→ InProgress（StartLevelSubST → WaitingSettle）→ Settlement
...
- **输入映射**：IMC 分层设计，详见 [输入与属性.md](输入与属性.md)
