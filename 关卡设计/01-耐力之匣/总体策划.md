...
| Actor | 职责 | 说明 |
|-------|------|------|
| BP_Puppet | 表现层 | 骨骼网格 + 动画，红绿灯切换时播放转身动画，发现玩家时播放 Anim_Detect |
...
| 动画 | 说明 |
|------|------|
| Anim_BackIdle | 背对玩家待机（循环） |
| Anim_FaceIdle | 面对玩家待机（循环） |
| Anim_Turn | 转身动画（启用 Root Motion，双向复用） |
...
**检测时机**：
- 木偶转身完成后（通过 Anim Notify 触发）
- GS.IsDetecting = true 时 Monitor 开始定时检测

**击杀流程**：
```
红灯 → Puppet 转身 → Anim Notify → GS.IsDetecting = true
    ↓
Monitor Line Trace → 命中 Monitor + Danger 标签
    ↓
Multicast_PlayerDetected → Puppet 播放 Anim_Detect
    ↓
对玩家造成伤害 → HP 归零 → 淘汰
```
...
**危险状态映射**：
- 行走、奔跑、蹲行、推搡、闪避、瞄准、投掷 → GE 添加 `Player.State.Danger`
...
- 奔跑：持续消耗
- 推搡/闪避/跳跃：按次消耗（GAS Cost）
- 自动回复：Periodic Gameplay Effect
...
| Comp_Endurance | 关卡1 操作组件（移动检测） | ✅ 已实现 | - |
| BP_Puppet | 木偶（表现层） | ✅ 已实现 | [BP_Puppet.md](核心类/BP_Puppet.md) |
| BP_Monitor | 监视器（逻辑层） | ✅ 已实现 | [BP_Monitor.md](核心类/BP_Monitor.md) |
| AN_DetectionStart | Anim Notify（检测开关） | ✅ 已实现 | - |
| BT_Endurance | 关卡1 AI 行为树 | ⚠️ 空壳已创建 | - |
| AbilitySet_Endurance | 关卡1 技能集 | ⚠️ 空壳已创建 | - |
| IMC_Endurance | 关卡1 非 GAS 输入（蹲、奔跑） | ❌ 待实现 | - |
| GE_Moving | 移动状态 GE（添加 Moving + Danger 标签） | ✅ 已实现 | - |
