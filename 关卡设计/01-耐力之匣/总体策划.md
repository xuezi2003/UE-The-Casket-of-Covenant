...
| **架构** | ST_Endurance | 子状态树（红绿灯） | ✅ 已实现 | [ST_Endurance.md](核心类/ST_Endurance.md) |
| **组件** | Comp_Char_Endurance | 移动检测/体力管理 | ✅ 已实现 | [Comp_Char_Endurance.md](核心类/Comp_Character_Endurance.md) |
...
| **数据** | AbilitySet_Endurance | 技能集配置 | ✅ 已实现 | - |
| **数据** | IMC_Endurance | 输入映射 | ✅ 已实现 | [输入映射.md](输入映射.md) |

**注意**：不需要 AIC_Endurance，使用通用的 AIC_Core + BT_Endurance 即可。

> [!NOTE]
> **状态机架构决策**：`ST_Endurance` 及其他涉及「状态」「阶段」切换的流程和物体，统一使用 **Logic Driver Lite** 实现（已满足项目需求：基础 FSM、网络复制、嵌套状态机、Any State、事件驱动转换）。AI 行为（`BT_Endurance`）继续使用 Behavior Tree。
...
| Phase 4 | Logic Driver 重构 | ❌ |
| Phase 5 | 场景搭建（PCG） | ❌ |
| Phase 6 | 伤害判定（扣血/淘汰/死亡广播） | ❌ |
| Phase 7 | AI 行为（BT_Endurance） | ❌ |
| Phase 8 | UI 完善（菜单/道具栏/美化） | ❌ |
| Phase 9 | 特效/音效 | ❌ |
