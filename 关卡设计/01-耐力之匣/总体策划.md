...
> [!NOTE]
> **状态机架构决策**：`ST_Endurance` 及其他涉及「状态」「阶段」切换的流程和物体，当前使用 **UE5 State Tree**，计划重构为 **Logic Driver Lite**（Phase 4）。AI 行为（`BT_Endurance`）继续使用 Behavior Tree。
...
| Phase 4 | Logic Driver 重构（State Tree → Logic Driver Lite） | ❌ |
| Phase 5 | 场景搭建（PCG） | ❌ |
