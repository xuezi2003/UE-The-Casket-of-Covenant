...
| 激活阻止标签 | `Player.Action.Aiming`, `Player.Action.Dodging`, `Player.Action.Pushing`, `Player.State.Exhausted`, `Player.State.Fallen`, `Player.State.Staggered`, `Player.State.Dead`, `Player.State.Attacked` |
...
```
CheckStamina (函数) → 返回 Bool (Stamina >= Abs(ConfigAsset.PushEffectContainer.SetByCallerMagnitude))
...
Play Montage and Wait for Event (Anim_Push_Montage)
    ...
    │       │       │              → 推搡者自己摔倒
    │       │       └─ 无 → Send Gameplay Event to Actor (目标, Gameplay.Event.Activate.Stagger.Push)
    │       │              → 目标进入失衡（播放蒙太奇）
    │       └─ 否 → 推空，无惩罚
    │
...
**设计说明**：
- **纯函数实现**：不执行操作，仅返回 `Stamina >= Cost` 的判断结果。
- **体力检查逻辑**：在 ActivateAbility 中调用，体力不足时通过 DelayUntilNextTick 延迟一帧再 EndAbility。
...
|--------|-----|
| 持续时间 | 无限（由 GA 手动移除） |
| Target Tags | `Player.State.Fallen` |
| 堆叠样式 | 按源聚合（限制1） |

**触发条件**：推搡命中无敌目标时，推搡者进入摔倒状态；或失衡超时。

> [!NOTE]
> **摔倒是部分安全状态**：只在蒙太奇播放时危险（通过 GE_Fallen_Danger 添加），倒地 Idle 时安全。

### GE_Fallen_Danger（摔倒危险标签）

| 配置项 | 值 |
|--------|-----|
| 持续时间 | 无限（由 GA 手动移除） |
| Target Tags | `Player.State.Danger` |
| 堆叠样式 | 按源聚合（限制1） |

**用途**：在 Fall Down 和 Fall GetUp 蒙太奇播放时添加危险标签，Fall Idle 期间不应用。
...
| 激活阻止标签 | `Player.State.Staggered`, `Player.State.Fallen`, `Player.State.Invincible`, `Player.State.Dead` |
| 取消带标签的能力 | `Player.Action.Aiming`, `Player.Action.Throwing`, `Ability.Require.Stamina` |
| 复制策略 | 请勿复制 |
| 实例化策略 | 每个 Actor 实例化 |
| 网络执行策略 | **服务器已启动** |
| 重新触发实例化能力 | ✅ |
...
| 触发标签 | 触发源 |
|----------|--------|
| `Gameplay.Event.Activate.Stagger.Push` | Gameplay事件（推搡/道具造成，播放蒙太奇） |
| `Gameplay.Event.Activate.Stagger.DeathBroadcast` | Gameplay事件（死亡广播造成，不播放蒙太奇） |

**Effect Container**：GE_Staggered, GE_StopMove
...
Event ActivateAbilityFromEvent
    ↓
SET EventData = K2_ActivateAbilityFromEvent.EventData
    ↓
Call Custom Event: ExecAbility()

---

Custom Event: ExecAbility
    ↓
...
CommitAbility → MakeEffectContainerSpecFromContainer (ConfigAsset.StaggerEffectContainer)
    ...
    ↓
Cast (GetAvatarActorFromActorInfo) To Character
    → GetController → SetIgnoreMoveInput (true)
    ↓
Switch on Gameplay Tag (EventData.EventTag)
    │
    ├─ 【Gameplay.Event.Activate.Stagger.DeathBroadcast】
    │   → Call Custom Event: ExecQTE()
    │
    └─ 【Gameplay.Event.Activate.Stagger.Push】
        → PlayMontageAndWaitForEvent (Anim_Stagger_Start_Montage)
            ├─ On Completed → Call Custom Event: ExecQTE()
            ├─ On Interrupted → Call Custom Event: ExecQTE()
            └─ On Cancelled → Call Custom Event: ExecQTE()

---

Custom Event: ExecQTE
    ↓
SET QTE_Finished = false
    ↓
...
Cast (GetController) to PC_Core
    ...
    │               → SendGameplayEvent (Gameplay.Event.Activate.Fall)
    │               → EndAbility
    │
    └─ 失败 (AI):
        → Delay(ConfigAsset.StaggerDuration × Random(0.5~1.0))
        → Random >= ConfigAsset.AISuccessRate?
            ├─ True → SendGameplayEvent (Gameplay.Event.Activate.Fall) → EndAbility
            └─ False → EndAbility (恢复)
...
**设计说明**：
- **SetIgnoreMoveInput**：启用/禁用玩家的移动输入（WASD/摇杆）。此设置只阻止 AddMovementInput 命令，不影响 Character Movement 组件的物理模拟或其他输入（如视角、攻击）。
```

**动画实现方式**：
- **Push 类型**：播放失衡开始蒙太奇（Anim_Stagger_Start_Montage）→ Stagger Idle（动画蓝图状态机）
- **DeathBroadcast 类型**：直接进入 Stagger Idle（动画蓝图状态机，不播放蒙太奇）
- Stagger Idle：通过 `Player.State.Staggered` 标签自动切换
...
| 激活阻止标签 | `Player.State.Fallen`, `Player.State.Invincible`, `Player.State.Dead` |
| 取消带标签的能力 | `Player.Action.Aiming`, `Player.Action.Throwing`, `Ability.Require.Stamina`, `Player.Action.Staggering` |
| 复制策略 | 请勿复制 |
| 实例化策略 | 每个 Actor 实例化 |
| 网络执行策略 | **服务器已启动** |
| 重新触发实例化能力 | ✅ |
...
**Effect Container**：GE_Fallen, GE_StopMove
...
Cast (GetAvatarActorFromActorInfo) To Character
    → GetController → SetIgnoreMoveInput (true)
    ↓
【阶段1：Fall Down 蒙太奇 - 有 Danger】
BP_ApplyGameplayEffectToOwner (GE_Fallen_Danger)
    → Array Add (Effect Container) ← 添加到数组，OnEndAbility 会清理
    ↓
PlayMontageAndWaitForEvent (Anim_Fall_Down_Montage)
    │
    ├─ 【On Completed】:
    │   【阶段2：Fall Idle - 无 Danger】
    │   RemoveActiveGameplayEffectBySourceEffect (GE_Fallen_Danger, -1)
    │       ↓
    │   Delay (ConfigAsset.FallDuration) ← 倒地不动，安全状态
    │       ↓
    │   【阶段3：Fall GetUp 蒙太奇 - 有 Danger】
    │   BP_ApplyGameplayEffectToOwner (GE_Fallen_Danger)
    │       ↓
    │   PlayMontageAndWaitForEvent (Anim_Fall_GetUp_Montage)
    │       ├─ 【Completed】 → RemoveActiveGameplayEffectBySourceEffect (GE_Fallen_Danger) → End Ability
    │       ├─ 【Blend Out】 → RemoveActiveGameplayEffectBySourceEffect (GE_Fallen_Danger) → End Ability
    │       ├─ 【Interrupted】 → RemoveActiveGameplayEffectBySourceEffect (GE_Fallen_Danger) → End Ability
    │       └─ 【Cancelled】 → RemoveActiveGameplayEffectBySourceEffect (GE_Fallen_Danger) → End Ability
    │
    ├─ 【On Interrupted】 → End Ability（OnEndAbility 会清理所有 GE）
    └─ 【On Cancelled】 → End Ability（OnEndAbility 会清理所有 GE）
...
**设计说明**：
- **Fall Down 蒙太奇**：身体倒下，移动中 → 有 Danger
- **Fall Idle**：倒地不动，静止 → 无 Danger（安全状态）
- **Fall GetUp 蒙太奇**：身体起身，移动中 → 有 Danger
- **OnEndAbility 兜底**：清理所有 Effect Container 中的 GE，包括 GE_Fallen_Danger
- **蒙太奇被打断**：Fall Down 被打断时直接 EndAbility，依赖 OnEndAbility 清理（罕见情况，实际影响很小）
...
- 整个摔倒过程**不可中断**（设计意图，但蒙太奇被打断时会直接结束）
...
| 激活阻止标签 | `Player.Action.Dodging`, `Player.Action.Pushing`, `Player.State.Exhausted`, `Player.State.Fallen`, `Player.State.Staggered`, `Player.State.Dead`, `Player.State.Attacked` |
| 复制策略 | 请勿复制 |
| 实例化策略 | 每个 Actor 实例化 |
| 网络执行策略 | 本地预测 |
| 重新触发实例化能力 | ✅ |
...
Event ActivateAbility
    ↓
CheckStamina (函数) → 返回 Bool (Stamina >= Abs(ConfigAsset.DodgeEffectContainer.SetByCallerMagnitude))
    ├─ False → DelayUntilNextTick → End Ability（不扣体力，延迟一帧避免 BTTask 卡死）
    └─ True ↓
...
**设计说明**：与 GA_Push 逻辑一致，详见 [体力系统.md](体力系统.md#激活前体力检查checkstamina-函数)。
...
> [!NOTE]
> ✅ **动作互斥已实现**：`GA_Push` 已配置 `Player.Action.Aiming` 阻止标签，详见 [动作互斥系统.md](动作互斥系统.md)。
