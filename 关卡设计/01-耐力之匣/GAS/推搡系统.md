...
> [!NOTE]
> **失衡是安全状态**：不添加 `Player.State.Danger` 标签，红灯时不会被判定死亡。
...
| 键 | GE |
|----|-----|
| `Effect.Container.Stagger` | GE_Staggered |

**变量**：
| 变量名 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| StaggerDuration | Float | 8.0 | QTE 超时时间 |
| AISuccessRate | Float | 0.5 | AI QTE 成功概率 (50%) |
...
Event ActivateAbilityFromEvent / Event ActivateAbility
    ↓
CommitAbility → Apply Effect Container (Effect.Container.Stagger) → SET Effect Container
    ↓
PlayMontageAndWaitForEvent (Anim_Stagger_Start_Montage)
    ...
    │       │
    │       └─ 失败 (AI):
    │           → Delay(StaggerDuration × Random(0.5~1.0))
    │           → Random >= AISuccessRate (0.5)?
...
| 键 | GE |
|----|-----|
| `Effect.Container.Fall` | GE_Fallen |

**变量**：
| 变量名 | 类型 | 说明 |
|--------|------|------|
| Fall Duration | Float | 躺地持续时间（5秒） |
| Effect Container | Array | 保存 GE Handle |
...
Event ActivateAbilityFromEvent
    ↓
CommitAbility → Apply Effect Container (Effect.Container.Fall) → SET Effect Container
    ↓
PlayMontageAndWaitForEvent (Anim_Fall_Down_Montage)
    │
    ├─ 【On Completed】 → Delay (Fall Duration = 5s)
    │       ↓
...
Check Stamina (宏) → Stamina >= 20?
    ├─ False → End Ability
    └─ True ↓
CommitAbility → Apply Effect Container (Effect.Container.Dodge) → SET Effect Container
    ↓
...
> **道具系统完成后统一设计**
> 
> 所有 Ability 的 Blocked Tags / Cancel Tags 将在道具系统完成后统一规划，当前阶段暂不处理互斥关系。
...
### 失衡系统
- [x] GE_Staggered（失衡状态，无 Danger）
- [x] GA_Stagger 基础框架（Event 触发 + Effect Container）
- [x] "失衡开始"蒙太奇（Anim_Stagger_Start_Montage）
- [x] Stagger Idle（动画蓝图状态机 Endurance_Stagger）
- [x] Cast to PC_Core 分支（玩家/AI 判断）
- [x] 玩家分支：Client_StartQTE + WaitGameplayEvent
- [x] 玩家超时处理（并行 Delay 8s + Client_CloseQTEWidget）
- [x] AI 概率判定（Random >= 0.5 → 摔倒）
- [x] Widget EndQTE 调用（QTE 成功后通知组件）

### 摔倒系统
- [x] GE_Fallen（摔倒状态，含 Danger）
- [x] GA_Fall 基础框架（Event 触发 + Effect Container）
- [x] "摔倒"蒙太奇（Anim_Fall_Down_Montage）
- [x] Delay 5s 趺地时间
- [x] "起身"蒙太奇（Anim_Fall_GetUp_Montage）
- [x] Fallen Idle（动画蓝图状态机 Endurance_Fall）

### 闪避系统
- [x] GE_Dodging（闪避状态，含 Danger）
- [x] GE_Invincible（无敌状态）
- [x] GA_Dodge 基础框架（Check Stamina + Effect Container）
- [x] 输入绑定（左 Alt）
- [x] AbilitySet_Endurance 添加
- [x] 闪避蒙太奇（Anim_Dodge_Montage）
- [x] 无敌帧事件通知（AN_GSC_SendGameplayEventByTag）
