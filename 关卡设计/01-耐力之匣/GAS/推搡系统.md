...
| 变量名 | 类型 | 说明 |
|--------|------|------|
| Stagger Duration | Float | 失衡持续时间（超时后触发 GA_Fall） |
...
PlayMontageAndWaitForEvent (Anim_Stagger_Start_Montage)
    │
    ├─ 【On Completed】 → Delay (Stagger Duration)
    │       ↓
    │   Send Gameplay Event (Gameplay.Event.Activate.Fall, Target=self) → End Ability
    │
...
- [x] 超时触发 GA_Fall（Delay + Send Gameplay Event）
- [ ] QTE 恢复逻辑（后期优化）
