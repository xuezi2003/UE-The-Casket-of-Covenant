...
Get Float Attribute (BAS_Core.Stamina)
    ↓
Return: Stamina >= Abs(ConfigAsset.XXXEffectContainer.SetByCallerMagnitude)
```

**关键**：
- 每个 GA 有一个 CheckStamina 纯函数，返回 Bool 值
- 使用 Abs() 处理负数 SetByCaller Magnitude（如 -20 → 20）
- 体力不足时使用 DelayUntilNextTick 延迟一帧再 EndAbility
- **DelayUntilNextTick 的作用**：避免 Ability 在同一帧内激活和结束，确保 BTTask_TriggerAbilityByClass 有时间绑定 OnAbilityEnded 委托（AI 使用的技能必须）
...
| 激活阻止标签 | `Player.State.Exhausted`, `Player.State.Staggered`, `Player.State.Fallen`, `Player.State.Dead`, `Player.State.Attacked` |
