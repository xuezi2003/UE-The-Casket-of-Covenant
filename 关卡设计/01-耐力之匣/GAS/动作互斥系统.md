...
| GA_Stagger | `Player.Action.Staggering` | `Staggered`, `Fallen`, `Invincible`, `Dead` | - | `Aiming`, `Throwing`, `Require.Stamina` | 服务器已启动 |
| GA_Fall | `Player.Action.Falling` | `Fallen`, `Invincible`, `Dead` | - | `Aiming`, `Throwing`, `Require.Stamina`, `Staggering` | 服务器已启动 |
...
| GE_StopMove | - | 无限 | 使 SpeedRate × 0，无堆叠策略 |
...
| GA_Fall | `Player.Action.Aiming`, `Player.Action.Throwing`, `Ability.Require.Stamina`, `Player.Action.Staggering` |
...
| GE | 配置 |
|----|------|
| **GE_StopMove** | Modifier: `BAS_Core.SpeedRate` × 0 <br> Duration: Infinite <br> Stack: None（无堆叠策略） |
...
> 
> **无堆叠策略**：允许多个来源（GA_Stagger、GA_Fall、GA_Attacked）同时应用 GE_StopMove，互不干扰。每个 GA 移除自己的 GE_StopMove 时不会影响其他 GA 应用的实例。
